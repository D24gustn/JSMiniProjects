    ...
    startBtn.addEventListener("click", function () { // 시작 버튼 클릭 시 실행되는 함수
        ...
        window.onkeydown = function (event) { // 키보드 이벤트 리스너 추가
            ...
            switch (event.key) { // 방향키에 따른 위치 이동 로직
                case 'ArrowLeft':
                    curLoc += (col > 0 ? -1 : size-1); // 왼쪽 이동, 왼쪽 끝에서 반대쪽 끝으로 이동
                    break;
                case 'ArrowRight':
                    curLoc += (col < size-1) ? 1 : -(size-1); // 오른쪽 이동, 오른쪽 끝에서 반대쪽 끝으로 이동
                    break;
                case 'ArrowUp':
                    curLoc += (row > 0 ? -size : (size-1) * size); // 위로 이동, 위쪽 끝에서 반대쪽 끝으로 이동
                    break;
                case 'ArrowDown':
                    curLoc += Number((row < size-1) ? size : -(size-1)*size); // 아래로 이동, 아래쪽 끝에서 반대쪽 끝으로 이동
                    break;
            }
            console.log(curLoc); // 현재 위치를 콘솔에 출력
